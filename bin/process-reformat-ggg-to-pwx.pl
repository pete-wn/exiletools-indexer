...
foreach my $message ( @$messages ) {
    if ( $message->valid ) {
      ...
      local %stash;
      try {
        %stash = %{JSON::XS->new->utf8->pretty->allow_nonref->decode($payload)};
      } catch {
        my $error = $_;
...
# === BEGIN Item Processing Code Chunk (this tag is used to extract this into other subroutines)
        $localChangeStats{TotalItems}++;
        $itemCount++;                                                              
        # Embedded format subroutine for speed
        local %item;
